...
var CartLine = function(item, count) {
    ...
    function orderCommon(self,orderParameters) {
        ...
        ko.utils.arrayForEach(self.lines(), function (line) {
            ...
            $.ajax({
                url: 'http://idiesh.ru/api/order/setItem/' + self.orderId + '/' + self.shopId + '/' + line.product()._id + '/' + line.quantity(),
                type: "GET",
                async: false,
                cache: false
    ...
    var Cart = function (orderId) {
        ...
        $.ajax({ url: 'http://idiesh.ru/api/order/' + orderId, cache: false, type: "GET" }).done(function (order) {
            ...
            ko.utils.arrayForEach(order.Items, function (item) {
                $.ajax({
                    url: "http://idiesh.ru/api/item/" + item.itemId,
                    type: "GET",
                    async: false,
                    cache: false
            ...
            $.ajax({
                url: "http://idiesh.ru/api/shops/" + order._shop,
                type: "GET",
                async: false,
                cache: false
            }).done(function (shop) {
